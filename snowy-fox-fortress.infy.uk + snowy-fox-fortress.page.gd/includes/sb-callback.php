...
if ($userbank->is_admin()) {
    ...
    $xajax->registerFunction("RemoveWhitelistRequest");
...
$xajax->registerFunction("Psignup");
...
function Psignup(string $username, string $password, string $redirect = '')
{
    global $userbank;
    $objResponse = new xajaxResponse();
    ...
    // Basic input validation
    if (empty($password)) {
        $objResponse->addRedirect('?p=signup&m=empty_pwd', 0);
        return $objResponse;
    }

    $pre = $GLOBALS['db']->Prepare("INSERT INTO " . DB_PREFIX . "_users(user,password,email) VALUES (?,?,?)");
    ...
    $objResponse->addRedirect('?p=login&m=signedup', 0);
    return $objResponse;
    // // Initialize session if not already started
    // if (session_status() === PHP_SESSION_NONE) {
    //     session_start();
    ...
    // $user = getUserFromDatabase($username);
    // if (!$user) {
    //     $objResponse->addRedirect("?p=signup&m=failed", 0);
    //     return $objResponse;
    // }
...
{
    $query = "SELECT aid FROM ".DB_PREFIX."_users WHERE user = ?";
...
function CheckSrvPassword($aid, $srv_pass)
{
    ...
    $res = $GLOBALS['db']->Execute("SELECT `srv_password` FROM `".DB_PREFIX."_users` WHERE `aid` = '".$aid."'");
...
function ChangeEmail($aid, $email, $password)
{
    ...
    $GLOBALS['db']->Execute("UPDATE `".DB_PREFIX."_users` SET `email` = ? WHERE `aid` = ?", array($email, $aid));
    $objResponse->addScript(
...
/**
 * @param int $sid Whitelist Request ID
 * @param int $archiv Action to perform.  1 for archive it, 0 for delete, 2 for restore

 * @return xajaxResponse
 */
function RemoveWhitelistRequest($sid, $archiv, $name, $profileid)
{
    $objResponse = new xajaxResponse();
    global $userbank, $username;
    if(!$userbank->HasAccess(ADMIN_OWNER|ADMIN_BAN_SUBMISSIONS)) {
        $objResponse->redirect("index.php?p=login&m=no_access", 0);
        Log::add("w", "Hacking Attempt", "$username tried to remove a whitelist request, but doesnt have access.");
        return $objResponse;
    }
    $sid = (int)$sid;
    ...
    $query1 = $GLOBALS['db']->Execute("DELETE FROM `" . DB_PREFIX . "_whitelistrequest` WHERE subid = $sid");

    if ($query1) {
        $objResponse->addScript("ShowBox('Whitelist Request Deleted', 'The selected whitelist request has been deleted from the database', 'green', 'index.php?p=admin&c=whitelist_requests', true);");
        Log::add("m", "Whitelist Request Deleted", "Whitelist Request ($sid) has been deleted.");

        
    } else {
        $objResponse->addScript("ShowBox('Error', 'There was a problem deleting the whitelist request from the database. Check the logs for more info', 'red', 'index.php?p=admin&c=whitelist_requests', true);");
    }
    return $objResponse;
}
...
function EditAdminPerms($aid, $web_flags, $srv_flags)
{
    ...
    // Update web stuff
    $GLOBALS['db']->Execute("UPDATE `".DB_PREFIX."_users` SET `extraflags` = $web_flags WHERE `aid` = $aid");
    ...
    // Update server stuff
    $GLOBALS['db']->Execute(
        "UPDATE `".DB_PREFIX."_users` SET `srv_flags` = ?, `immunity` = ? WHERE `aid` = $aid",
    ...
    $admname = $GLOBALS['db']->GetRow("SELECT user FROM `".DB_PREFIX."_users` WHERE aid = ?", array((int)$aid));
...
function EditGroup($gid, $web_flags, $srv_flags, $type, $name, $overrides, $newOverride)
{
    ...
    if($type == "srv") {
        ...
        $oldname = $GLOBALS['db']->GetAll(
            "SELECT aid FROM " . DB_PREFIX . "_users WHERE srv_group = ?",
            array($gname['name'])
        );
